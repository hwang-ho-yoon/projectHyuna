...
<script type="text/javascript">
	...
	$(function() {
		...
		$(".ogr_cancel").click(function() {
			...
			$("#rncFrm").attr({
				"method" : "post",
				"action" : "/orderAdmin/orderAdminCancel.do"
			});
			$("#rncFrm").submit();
		});
		$(".ogr_recall").click(function() {
			...
			$("#rncFrm").attr({
				"method" : "post",
				"action" : "/orderAdmin/orderAdminRecall.do"
			});
			$("#rncFrm").submit();
		});
		$(".rncDetail").click(function() {
			group_no = $(this).parents("tr").children().eq(0).attr("data-num");
			var url = "/orderAdmin/all/" + group_no + ".do";
	...
	<div id="wrapper">
		<form id="rncFrm">
			<input type="hidden" id="no" name="ogr_no" value="0">
		</form>
					...
					<c:when test="${not empty orderGroups}">
						<c:forEach var="orderGroup" items="${orderGroups}" varStatus="Groupstatus">
							<tr> 
								...
								<td>
									<c:choose>
										<c:when test="${orderGroup.ogr_state eq 'standby_cancel'}">
											<button class="btn btn-default btn-sm ogr_cancel" style="margin: 0">취소승인</button>
										</c:when>
										<c:when test="${orderGroup.ogr_state eq 'standby_recall'}">
											<button class="btn btn-default btn-sm ogr_recall" style="margin: 0">반품승인</button>
										</c:when>
									</c:choose>
								</td>
							</tr>
